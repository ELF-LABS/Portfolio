...
3. **EverMemOS** (Spark:1995 via Tailscale 100.106.64.62) — shared Coven memory
...
**1c. EverMemOS Check**
```bash
curl -s http://100.106.64.62:1995/api/v1/memories/search -H "Content-Type: application/json" \
...
```bash
curl -s -X POST http://100.106.64.62:1995/api/v1/memories \
...
- Endpoint: `http://100.106.64.62:1995/api/v1/memories` (Tailscale IP — UFW blocks LAN)
