...
Pilot evaluations used a Qwen3.5-35B local judge as primary, with cloud cross-family judges (Llama 3.3 70B via Groq; GLM-4.7-Flash via Z.ai) where rate-limits permitted. Most cells fell back to single-judge under cloud-judge timeouts. Comparison between subjects within each cell is preserved (same judge, same prompt); absolute scores carry less cross-cell calibration than a triple-judge ensemble would have provided.

**Inference settings (all subjects, all judges, all runs):** All subjects (`pattern`, `pattern_candidate`, `coven`) were served via SGLang at **temperature=0.0** (deterministic / greedy decoding), `max_tokens=1024`, `chat_template_kwargs={"enable_thinking": False}` (Qwen3.5 freeform path); `top_p` and `top_k` were left at framework defaults, which are moot under greedy decoding. All judges ran at **temperature=0.0** (local Qwen3.5-35B and Groq Llama 3.3 70B) or `do_sample=False` (Z.ai GLM-4.7-Flash, which bypasses temperature/top_p), `max_tokens=600`, JSON-structured output where the endpoint supports it. **Three independent runs per cell**; observed σ across runs (0.0–1.0) measures *judge stability* (cloud judges retain minor non-determinism at these settings), not subject sampling variance — the subjects are fully deterministic under greedy decoding.

Significance is reported via Welch *t*-tests with Cohen's *d* effect sizes; per-domain results with small *n* are reported as directional and explicitly flagged. The compositional-subject (`coven`) path queried a Milvus instance that did not contain the research-corpus collection during the run; same-degraded-compositional on both pipelines preserves the comparison. Future work: powered replication with research-corpus enrichment.
